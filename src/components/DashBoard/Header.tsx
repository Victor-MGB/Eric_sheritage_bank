...
import CurrencyDropDwn from "../../hooks/CurrencyDropDwn";
...
const Header: React.FC<{ handleShowSide: () => void }> = ({ handleShowSide }) => {
  ...
  //fetch notification on cmomponent
  useEffect(() => {
  ...
  return (
    <header className="bg-gray-50 rounded-xl md:w-full md:max-w-full shadow-md py-4 md:px-6 px-1 flex justify-between items-center mb-4">
      ...
      >
        ...
        <CurrencyDropDwn getter={getter} />
      </div>

      <FaBars
        className={
          "text-gray-800 text-4xl font-bold hover:text-neutral-600 cursor-pointer md:hidden "
        }
