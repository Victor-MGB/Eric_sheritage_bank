...
const Main: React.FC = () => {
	...
	//on mount, set the balance with balance data from usercontext value
	useEffect( () => {
...
const BalanceOverview: React.FC<BalanceOverviewProps> = ( { balance, handleStage } ) => {
	const [localCurrencyRate, setlocalCurrencyRate] = useState( 0 );
	const [withdrawalOpen, setWithdrawalOpen] = useState<boolean>( false );
	const [sendOpen, setSendOpen] = useState<boolean>( false );

	const UserCurrencyName: string | undefined = useContext( UserDataCOntext )?.currency;

	// console.log('userCurrency',UserCurrencyName);
	...
	//fetch the converted local equivalent currency from session storage
	useEffect( () => {
		const localCurrencyRate = sessionStorage.getItem( "conversionResult" );
		if ( localCurrencyRate ) {
			setlocalCurrencyRate( JSON.parse( localCurrencyRate ) );
		}
	}, [localCurrencyRate] );
	...
	return (
		<div className='bg-white p-6 rounded-lg shadow-md mb-6 md:w-full w sm:flex flex-wrap justify-between items-center'>
			...
			<div className='flex items-center justify-start gap-6 mb-6 md:mb-3 bg-green-50 h-fit md:h-[100%] sm:w-[35rem] rounded-lg p-6 '>
				<div className='flex flex-col'>
					<div className='flex items-center mb-2 p-2 gap-3'>
						<FaWallet className='mr-2 text-sm text-blue-500' />
						<h2 className='text-sm font-semibold capitalize'> int-baseCurrency Balance</h2>
						<p className='text-xl font-bold'>${ balance }</p>
					</div>

					<div className='flex items-center mb-2 p-2 gap-3'>
						<FaWallet className='mr-2 text-sm text-blue-500' />
						<h2 className='text-sm font-semibold capitalize'> local-Currency Balance</h2>
						<p className='text-xl font-bold'>{ UserCurrencyName } { localCurrencyRate }</p>
					</div>
				</div>
				...
				<div className='flex flex-col self-start'>
					<div className='flex items-center mb-2'>
						<FaWallet className='mr-2 text-sm text-blue-500' />
						<h2 className='text-sm font-semibold capitalize'>Loan Balance</h2>
					</div>
	...
	return (
		...
		>
			...
			<div className='bg-white w-full p-6 rounded-lg shadow-md mb-6'>
				...
				<form
					className={ `h-0 overflow-hidden transition-all duration-700 w-[20rem] ${ state ? "h-full duration-500 transition-all p-3" : "h-0 duration-500 transition-all"
						}` }
				>
					<div className='mb-4'>
